...
(async () => {
  const rows = csvRows("./data/vehicles.csv");
  let wrote = 0;
  for (const r of rows) {
    const make = r.make; const year = Number(r.year);
    if (!make || !year) continue;
    const ref = db.doc(`makes/${make}/years/${year}/vehicles/${(r.model||"").replace(/[\s\/]/g,"_") || "base"}`);
    await ref.set({
      make, year, model: r.model || "", trim: r.trim || "", category: r.category || "Vehicle"
    }, { merge: true });
    wrote++;
  }
  console.log("Seeded vehicles:", wrote);
})();